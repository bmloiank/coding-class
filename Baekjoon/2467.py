...
# -99 98  합이 0보자 작으면 알카리(-) 수치가 작은걸로(start 더 오른쪽)
# -2 98   합이 0보다 커  산성(+) 더 작은거로(end 더 왼쪽)
# -2 4
# -1 -2 => 체크 필요없음
...
summ = abs(deck[start] + deck[end])
# answer = [deck[start], deck[end]]
...
while True:
    ...
    tsumm=abs(deck[start] + deck[end])

    # 
    if deck[start] + deck[end] > 0:
        if tsumm < summ:
            summ = tsumm
            startv = deck[start]
            endv = deck[end]  
